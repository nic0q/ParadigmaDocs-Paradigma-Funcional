...
; LOGIN:
; Descripción: Funcion agnostica que logea al usuario en paradigmadocs, si sus credenciales son correctas (username y password)
...
(define (create paradigmadocs)
  (λ(date nombre contenido)
    (define (primer_documento pDocs date title contenido creador)
      (set_documento pDocs (aniadir_doc (documento title creador (set_id_doc pDocs) (list (version 0 date contenido))null)(get_documentos pDocs))))
...
; No se puede comentar una misma versión ya comentada 2 veces, en caso exista un comentario, se comenta la version inicial sin comentario
; Dominio: paradigmadocs X int X date X String X String
; Recorrido: paradigmadocs
...
; Dominio paradigmadocs X int X int
; Recorrido: paradigmadocs
; Tipo de Recursividad: Recursividad de cola (Función actualizarvz->get_n_version)
