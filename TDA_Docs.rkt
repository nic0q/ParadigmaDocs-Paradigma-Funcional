...
(define (get_doc_byId pDocs idDoc)
  (if (null? (filter (lambda (x) (equal? idDoc (get_id_documento x)))(get_documentos pDocs)))
      null
      (car(filter (lambda (x) (equal? idDoc (get_id_documento x)))(get_documentos pDocs)))))
...
(define (set_version pDocs idDoc x)
  (set_documento pDocs (append (filter(lambda (x) (not(eqv? idDoc (get_id_documento x))))(get_documentos pDocs))(list(set_version_to_doc pDocs idDoc x)))))
...
(define (aniadir_permisos pDocs idDoc permisos)
  (append (list(reverse(append (list permisos) (cdr (reverse(get_doc_byId pDocs idDoc))))))(filter (lambda (x) (not(equal? (get_id_documento x) idDoc)))(get_documentos pDocs))))
...
; Para esta funcion es necesario crea una nueva funcion que restaure solo el texto de
(define (restaura_version lista x v)
  (if (empty? lista)
      null
      (if (eqv? x v)
          (car lista)
          (restaura_version (cdr lista) (+ x 1) v))))
