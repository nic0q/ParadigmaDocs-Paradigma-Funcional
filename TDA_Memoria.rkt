...
(define (actualizar_vz pDocs idDoc number_of_undo)  ;  BUG: Si  number_of_undo   =   0
  (if (tiene_memoria? pDocs idDoc)
      (list (get_id_version (get_active_version_byid pDocs idDoc))(get_date_version (get_active_version_byid pDocs idDoc)) (get_texto_version (restaura_version (get_historialDoc_byid pDocs idDoc) 0 ( + (get_memory_state pDocs idDoc)number_of_undo))) (list (get_text_memory pDocs idDoc)(+ (get_memory_state pDocs idDoc)number_of_undo)))
      (list (get_id_version (get_active_version_byid pDocs idDoc))(get_date_version (get_active_version_byid pDocs idDoc)) (get_texto_version (restaura_version (get_historialDoc_byid pDocs idDoc) 0 number_of_undo))(crear_memoria pDocs idDoc number_of_undo) )))
...
(define (actualizar_vy pDocs idDoc number_of_redo)
  (if (tiene_memoria? pDocs idDoc); Se comprueba que se haya hecho CtrlZ antes
      ...
      (list (get_id_version (get_active_version_byid pDocs idDoc))(get_date_version (get_active_version_byid pDocs idDoc)) (get_texto_version (restaura_version (get_historialDoc_byid pDocs idDoc) 0 ( - (get_memory_state pDocs idDoc)number_of_redo))) (list (get_text_memory pDocs idDoc)(- (get_memory_state pDocs idDoc)number_of_redo))))
      pDocs))
...
; GET_TEXT_STATE: Función que obtiene el texto original de la  versión donde se hizo CtrlZ por primera vez
; Dominio: paradigmadocs X int
; Recorrido: string
