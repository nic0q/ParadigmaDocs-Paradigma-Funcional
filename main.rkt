...
(define (restoreVersion paradigmadocs idDoc idVersion)
  (define (actualizar_documento)
    ...
    (append (filter (lambda (x) (equal? idVersion (get_id_version x))) (get_historialDoc_byid paradigmadocs idDoc))
     ...
     (filter (lambda (x)(not(equal? idDoc (get_id_documento x))))(get_documentos paradigmadocs)))))
  (if (logeado? paradigmadocs)
...
(define (revokeAllAccesses paradigmadocs)
  (define (actualizar_documento)
    (if (empty? (filter (lambda (x) (equal? (get_logeado paradigmadocs) (get_autor x))) (get_documentos paradigmadocs)))
        paradigmadocs 
        (set_documento paradigmadocs (append (filter (lambda (x) (not(equal? (get_logeado paradigmadocs) (get_autor x)))) (get_documentos paradigmadocs)) 
                                                   (map eliminar_permisos (filter (lambda (x) (equal? (get_logeado paradigmadocs) (get_autor x))) (get_documentos paradigmadocs)))))))
...
; DOCUMENTOS->STRING: Función encapsulada  que obtiene una versión 'string' de toda la información de los documentos de paradigmadocs, esta funcion recibe datos de
...
(define (paradigmadocs->string paradigmadocs)
  ...
  (string-join(map (lambda (nombre_doc creador id historial shares)
                   ...
                   (map (lambda (x) (get_lista_accesos x))lista_documentos))))
  (if (logeado? paradigmadocs)
...
(define gDocs180 ((login gDocs17 "user2" "pass2" comment ) 2 (date 22 10 2021)  "um"  "comentarioooo")) ;En que caso se quiera comentar una version ya antes comentada, se comentara la ultima version sin comentar del documento
;(define a ((login gDocs00014  "user2" "pass2" comment ) 1 (date 20 10 2021)  ""  "comentarioo")) ; se comenta un string vacio
;gDocs17
;gDocs180
;(display(paradigmadocs->string gDocs180))

; APPLYSTYLES:
; Dominio: paradigmadocs X int X date X String X char List
; Recorrido: paradigmadocs
; Condiciones de funcionamiento:
; 1) Los permisos otorgados solo pueden ser los characters (#\i #\b #\u)
; 2) 
  ...
  (if (logeado? paradigmadocs)
      (if (and (es_id? idDoc)(not(eqv? searchText ""))(date? date)(es_texto? searchText)(member (get_logeado paradigmadocs )(get_editor_users paradigmadocs idDoc)))
          (if (string-contains? (get_contenido_active_version paradigmadocs idDoc) ((get_function2 paradigmadocs)searchText)) ; Si encuentra texto
              ...
              ; por algun motivo, crea 2 "" al principio y al final, por eso reverse y cd.r)
              (deslogear paradigmadocs))
          (deslogear paradigmadocs))
      paradigmadocs))
...
;(display(login gDocs18 "user1" "pass1" paradigmadocs->string)), al mostrar el d
...
; SET_VERSION_SIN_CREAR: Función que actualiza la versión sin crear una nueva
...
(define (set_doc pDocs idDoc x)
  (append (filter(lambda(x)(not(eqv? idDoc (get_id_documento x))) ) (get_documentos pDocs)) x))

; CtrlZ: Funciónq que deshace los cambios hechos en  la versión activa del documento, esta función crea una memoria  que guarda los datos de la versión donde se deshaceran
...
; Recorrido: paradigmadocs
(define (ctrlZ pDocs idDoc number_of_undo)
  (define (crear_documento update)
    (set_documento pDocs (set_doc pDocs idDoc (list(update_doc pDocs idDoc update)))))
  (crear_documento (actualizar_vz pDocs idDoc number_of_undo)))

; CtrlY: Función que rehace los cambios hechos por CtrlZ, para su funcionamiento se debe hacer CtrlZ antes
; Dominio paradigmadocs X int X int
; Recorrido: paradigmadocs
(define (ctrlY pDocs idDoc number_of_redo)
  (define (crear_documento update)
    (set_documento pDocs (set_doc pDocs idDoc (list(update_doc pDocs idDoc update)))))
  (crear_documento (actualizar_vy pDocs idDoc number_of_redo)))

;-------------------------------EJEMPLOS---------------------------------
;En  este ejemplo se aplica  CtrlZ  sobre el mismo documento (misma version) 2 veces
(define  hybryd1  (ctrlY (ctrlZ (ctrlZ gDocs16 0 1) 0 2)0 3)) 
(define  hybryd2 (ctrlY(ctrlZ gDocs16 0 3) 0 3))
hybryd1
hybryd2
